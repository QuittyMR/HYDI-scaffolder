...
class App(object):
    def __init__(self):
        ...
        self._allowed_domain = self.app.config.get('cors', 'allowed_domain')
    ...
    def run(self):
        ...
        server.route('/api/<any:path>', method='OPTIONS', callback=self._add_cors_headers)

        for source in self._api_sources:
            self._register_routes(server, source)

        server.run(**dict(self.app.config.items('server')))
